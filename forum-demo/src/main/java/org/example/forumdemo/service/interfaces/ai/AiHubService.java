...
public interface AiHubService {
    ...
    java.util.List<Long> ragVectorSearchArticles(String query, java.util.List<Map<String, Object>> candidates);

    /**
     * 向量召回（qwen3-vl-embedding），返回 [{articleId, score}, ...] 按 score 降序。
     * candidates 可为空，仅走 Redis 全库向量扫描。
     */
    java.util.List<Map<String, Object>> ragArticleVectorRanked(
            String query, java.util.List<Map<String, Object>> candidates);

    /** 用户 AI 语义搜索：Redis 向量召回；失败返回空列表 */
    java.util.List<Long> ragVectorSearchUsers(String query, java.util.List<Map<String, Object>> candidates);
}
