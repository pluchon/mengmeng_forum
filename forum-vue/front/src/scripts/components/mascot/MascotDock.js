...
export function useMascotDock() {
  ...
  // 这条流是发在哪个会话上的（空串表示没有在途的流）。
  // 用会话 id 而不是「第几次换视图」的计数：计数在切走时 +1、切回来又 +1，
  // 永远回不到原来的值，于是切回去反而认不出这条流是自己的。
  ...
  const streamInCurrentView = computed(
    () => !!streamSessionId.value && String(streamSessionId.value) === String(sessionId.value),
  )
  ...
  function messagesForPersist(rows) {
    return (rows || []).filter((m) => !(m?.streaming && !String(m?.content || '').trim()))
      .map((m) => {
        if (!m?.streaming && !m?.thinkingText) return { ...m, _streamKey: undefined }
        return { ...m, streaming: false, thinkingText: '', _streamKey: undefined }
      })
  }
  ...
  async function sendInternal(text, { skipPushUser = false } = {}) {
    ...
    // 本轮流认哪个会话。切走时界面写入全部丢弃，切回来再把同一条气泡挂回去。
    let streamSid = String(sessionId.value || '')
    const isStale = () => String(sessionId.value || '') !== streamSid
    streamSessionId.value = streamSid

    const skill = activeNav.value === 'drawing' ? 'drawing' : 'chat'
    let streamHadError = false
    let streamProducedImage = false
    // 不能存下标：切会话时 messages 会被整体替换，下标会指到别人身上。
    // 也不能只存对象引用：改原始对象不触发视图更新，必须从数组里取回代理。
    const rowKey = `stream-${now}-${Math.random().toString(16).slice(2)}`
    let rowSeed = null

    function liveRow() {
      if (!rowSeed || isStale()) return null
      let index = messages.value.findIndex((m) => m?._streamKey === rowKey)
      if (index < 0) {
        // 切走又切回来了：把这条气泡（带着已经流出来的内容）重新挂上
        messages.value.push(rowSeed)
      ...
      await new Promise((resolve, reject) => {
        chatStreamAbort = streamMascotChat(
          ...
          },
          {
            ...
            onMeta(meta) {
              if (isStale()) return
              const sidBefore = String(sessionId.value || '')
              applyServerSessionId(meta)
              const sidAfter = String(sessionId.value || '')
              // 本地临时 id 首次落库会被换成服务端 id；这条流跟着改，
              // 否则下一次 isStale() 会把自己判成「用户切走了」
              if (sidAfter !== sidBefore && sidBefore === streamSid) {
                streamSid = sidAfter
                streamSessionId.value = sidAfter
              }
              const row = liveRow()
            ...
            onError(msg) {
              ...
              if (row) {
                ...
                row.thinkingText = ''
                if (!row.content?.trim()) {
                  const index = messages.value.findIndex((m) => m?._streamKey === rowKey)
  ...
  async function send() {
    ...
    const draftSid = String(sessionId.value || '')
    try {
      await sendInternal(text)
    } catch {
      if (draftSid === String(sessionId.value || '')) draft.value = text
    }
  }
