...
/** 已下线字典管理页：后端旧菜单仍含 /system/dict 时在此剔除 */
function omitDictManageMenu(items: ListItem[]): ListItem[] {
  const bannedPath = '/system/dict'
  return items
    .filter(it => it.path !== bannedPath && it.title !== '字典管理')
    .map((it) => {
      if (!it.children?.length)
        return it
      return { ...it, children: omitDictManageMenu(it.children) }
    })
}
...
const storeSetup = () => {
  ...
  const generateRoutes = async (): Promise<RouteRecordRaw[]> => {
    try {
      ...
      const menus = omitDictManageMenu(cloneDeep(res.data ?? []))
      const asyncRoutes = formatAsyncRoutes(menus)
