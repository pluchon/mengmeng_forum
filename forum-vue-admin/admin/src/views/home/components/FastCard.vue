<template>
  <div class="fast-card">
    <a-row :gutter="[14, 14]">
      <a-col v-for="item in list" :key="item.path" v-bind="colProps">
        <div class="fast-card-item" role="button" tabindex="0" @click="go(item.path)" @keydown.enter="go(item.path)">
...
const list = [
  { name: '帖子管理', icon: 'icon-file', path: '/content/article', color: '#377DFF' },
  { name: '用户管理', icon: 'icon-user', path: '/system/user', color: '#00B7FF' },
]
</script>
