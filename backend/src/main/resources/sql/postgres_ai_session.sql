...
-- 本文件可重复执行。已有库仅需跑下方 ALTER / 索引 / 触发器段时，也可单独执行同目录 incremental_postgres_ai_session.sql。
...
-- MySQL forum_ai_usage_log.related_id 可与 ai_chat_session.id / client_request_id 对齐做计费幂等。
--
-- 可选独立 schema：
