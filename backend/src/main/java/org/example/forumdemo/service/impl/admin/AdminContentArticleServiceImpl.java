...
public class AdminContentArticleServiceImpl implements AdminContentArticleService {
    ...
    private List<AdminArticlePreviewCommentVO> loadTopComments(Long articleId) {
        List<ArticleReply> replies = articleReplyMapper.selectPage(new Page<>(1, 10, false),
                Wrappers.lambdaQuery(ArticleReply.class)
                ...
                .orderByDesc(ArticleReply::getCreateTime)).getRecords();
        if (replies.isEmpty()) {
            return List.of();
        }
