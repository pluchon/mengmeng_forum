...
# 动图审核用：抽样帧被拼成一张网格图送来。不说明的话模型只会描述最显眼的那一格，
# 藏在其它格里的违规内容就漏掉了——而这正是动图需要抽帧的原因
ANIMATION_DESC_PROMPT = (
    "这是同一个动图的多个连续画面, 按从左到右、从上到下的顺序排列. "
    "请逐格查看每一个画面, 只要任意一格出现不适宜内容就必须指出来. "
    "请用一句话概括所有画面的核心内容, 并给出 3~6 个关键词, 用逗号分隔. "
    "不需要额外解释, 只输出描述和关键词即可."
)
