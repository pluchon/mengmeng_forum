"""集中读取 config.yaml，并由环境变量覆盖密钥与机器相关配置。"""
...
class Settings:
    def __init__(self, raw: dict[str, Any]):
        self.raw = raw
        # 密钥只允许由进程环境覆盖，配置文件仅保存非敏感默认值
        ds = raw.get("dashscope", {})
        ds["api_key"] = env_nonempty("DASHSCOPE_API_KEY", ds.get("api_key"))
        rd = raw.get("redis", {})
        rd["password"] = env_nonempty("REDIS_PASSWORD", rd.get("password"))
        rb = raw.get("rabbitmq", {})
        ...
        raw["rabbitmq"] = rb
        pg = raw.get("postgres", {})
        pg["host"] = env_nonempty("POSTGRES_HOST", pg.get("host"))
        pg["port"] = env_nonempty("POSTGRES_HOST_PORT", env_nonempty("POSTGRES_PORT", pg.get("port")))
        pg["user"] = env_nonempty("POSTGRES_USER", pg.get("user"))
        pg["password"] = env_nonempty("POSTGRES_PASSWORD", pg.get("password"))
        pg["db"] = env_nonempty("POSTGRES_DB", pg.get("db"))
        raw["postgres"] = pg
